...
\item What matters the most among the three types of motions is the bridge motions along the $x$ axis. The zoomed-in plots in this time window are shown in Fig.\,\ref{fig:amc_crane}\,(b).  The locations of the bridge at different times are annotated in the boxes in the figure. It can be seen that the field variations produced is reasonably reproducible and adds $\Delta \mathbf{B}\approx (+6\,\mu\mathrm{T}, 0\,\mu\mathrm{T},+2\,\mu\mathrm{T})$ when the bridge is right above the TUCAN area. The inhomogeneity of the field is evaluated to be  $\Delta B_{i}/\Delta x_{j} < \left|1.7 \mu \mathrm{T}/ \mathrm{m}\right|\,\,( B_{i}= \{B_{x}, B_{y}, B_{z}\}, x_{j}=\{x,y,z\})$.
\end{itemize}
